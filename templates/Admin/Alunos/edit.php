    ...
    <div class="card-body">
        ...
        <div class="row">
            <div class="col-md-4">
                ...
                <?= $this->Form->file('arquivo_rg', ['class' => 'form-control-file']); ?>
            </div>
            <div class="col-md-2 text-center">
                <label>Visualizar R.G.</label>
                <?= $aluno->arquivo_rg ? '<a class="btn btn-default btn-block" target="_blank" href="' . $this->Url->build('/uploads/arquivos/' . $aluno->cpf . '/' . $aluno->arquivo_rg) . '"><i class="fa fa-file-pdf-o"></i> Visualizar</a>' : 'Não'; ?>
            </div>
            <div class="col-md-4">
                <label for="arquivo_atestado">Anexar cópia do CPF</label>
                <?= $this->Form->file('arquivo_cpf', ['class' => 'form-control-file']); ?>
            </div>
            <div class="col-md-2 text-center">
                <label>Visualizar cópia do CPF</label>
                <?= $aluno->arquivo_cpf ? '<a class="btn btn-default btn-block" target="_blank" href="' . $this->Url->build('/uploads/arquivos/' . $aluno->cpf . '/' . $aluno->arquivo_cpf) . '"><i class="fa fa-file-pdf-o"></i> Visualizar</a>' : 'Não'; ?>
            </div>
        ...
        <div class="row">
            ...
            <div class="col-md-4 text-center">
                <label>Visualizar Comprovante de Residência</label>
                <?= $aluno->arquivo_comprovante_residencia ? '<a class="btn btn-default btn-block" target="_blank" href="' . $this->Url->build('/uploads/arquivos/' . $aluno->cpf . '/' . $aluno->arquivo_comprovante_residencia) . '"><i class="fa fa-file-pdf-o"></i> Visualizar</a>' : '<br /> Não anexado'; ?>
            </div>
        </div>
    ...
    <div class="card-body">
        ...
        <div class="row">
            ...
            <div class="col-md-2 text-center">
                ...
                <?= $aluno->alunos_saude->arquivo_atestado ? '<a class="btn btn-default btn-block" target="_blank" href="' . $this->Url->build('/uploads/arquivos/' . $aluno->cpf . '/' . $aluno->alunos_saude->arquivo_atestado) . '"><i class="fa fa-file-pdf-o"></i> Visualizar</a>' : 'Não'; ?>
            </div>
            <div class="col-md-4">
                <?= $this->Form->control('alunos_saude.validade_atestado', ['class' => 'form-control']); ?>
            </div>
        </div>
    </div>
</div>
...
<div class="card">
    <div class="card-body">
        ...
        <div class="form-check">
            <?= $this->Form->control('declaro', ['type' => 'checkbox', 'class' => 'form-check-input checkbox-large', 'label' => 'Declaro que estou ciente de que é obrigatória a entrega do atestado médico no prazo de 30 dias para a prática de atividade física, assumindo plena responsabilidade pela veracidade das informações acima.', 'checked']); ?>
        </div>
    </div>
</div>
<div class="form-check">
    <?= $this->Form->control('autorizo', ['type' => 'checkbox', 'class' => 'form-check-input checkbox-large', 'label' => 'Autorizo expressamente a utilização de sua imagem e voz, em caráter definitivo e gratuito, constante em fotos e filmagens decorrentes de sua participação no projeto', 'checked']); ?>
</div>
<div class="form-check">
    <?= $this->Form->control('atesto', ['type' => 'checkbox', 'class' => 'form-check-input checkbox-large', 'label' => 'Atesto que todas as informações fornecidas são verdadeiras.', 'checked']); ?>
</div>

<?= $this->Form->control('observacoes', ['label' => 'Observação/Justificativa de falta', 'class' => 'form-control tiny']); ?>
...
<script>
    ...
    $('[id$="qual"]').parent().hide();
    // Verifica os campos já marcados como "Sim" ao carregar a página
    $('.verifica_sim:checked').each(function () {
        if ($(this).val() == 1) {
